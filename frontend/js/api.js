...
const API_BASE_URL = isLocalhost
    ...
    : 'https://english-dictionary-api-h6e2.onrender.com/api';
...
async function searchWord(word) {
    ...
    try {
        response = await fetch(
            `${API_BASE_URL}/words/${encodedWord}`
        );
    } catch (networkErr) {
        ...
        console.error(
            '[api] Network error while fetching word:',
            networkErr
        );
    ...
    if (!response.ok) {
        ...
        console.error(
            `[api] Client error: HTTP ${response.status}`
        );
    ...
    try {
        ...
        throw {
            ...
            message:
                "We couldn't load the dictionary result. Please try again."
        };
    }
    ...
    if (dictionaryCache.size >= MAX_CACHE_SIZE) {
        ...
        dictionaryCache.delete(
            dictionaryCache.keys().next().value
        );
    }
...
async function getSuggestions(prefix) {
    ...
    try {
        const response = await fetch(
            `${API_BASE_URL}/words/suggest/${encodeURIComponent(trimmed)}`
        );
        ...
        console.error(
            '[api] Suggestions request failed:',
            err
        );
